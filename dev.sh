...
MAILPIT_CONTAINER="mailpit.$NETWORK"
COUCHDB_PORT=5984
REDIS_PORT=6379
...
if port_open $MAILPIT_SMTP_PORT; then
    echo "Mailpit already accessible on localhost:$MAILPIT_SMTP_PORT — skipping restart."
else
    echo "Starting Mailpit (SMTP port $MAILPIT_SMTP_PORT, web UI port $MAILPIT_UI_PORT)..."
    docker rm -f "$MAILPIT_CONTAINER" 2>/dev/null || true

    docker run --name "$MAILPIT_CONTAINER" -d \
        --network "$NETWORK" \
    ...
    until nc -z -w1 localhost "$MAILPIT_SMTP_PORT" 2>/dev/null; do
        sleep 1; echo -n "."; done
    echo " ready.  Inbox: http://localhost:$MAILPIT_UI_PORT"
fi
...
# SMTP: point at local Mailpit; no auth, no TLS
export GROUCH_SMTP_HOST=localhost
export GROUCH_SMTP_PORT=$MAILPIT_SMTP_PORT
export GROUCH_SMTP_FROM=dev@localhost
export GROUCH_SMTP_TLS=false
